...
extern "C" {
#endif
void _sphemultipole_P_S(const Libint_t* inteval) {
...
_sphemultipole_P_S_prereq(inteval+c, inteval->stack);
}
...
inteval->targets[21] = &(inteval->stack[((hsi*3+63)*1+lsi)*1]);
inteval->targets[22] = &(inteval->stack[((hsi*3+66)*1+lsi)*1]);
inteval->targets[23] = &(inteval->stack[((hsi*3+69)*1+lsi)*1]);
inteval->targets[24] = &(inteval->stack[((hsi*3+72)*1+lsi)*1]);
/** Number of flops = 0 */
}
