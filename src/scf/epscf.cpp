      ...
      for (auto const &[quantum_part_b_key, quantum_part_b] :
        ...
        for (size_t i = 0; i < num_basis; i++) {
          for (size_t j = 0; j < num_basis; j++) {
            double matrix_elem_alpha = 0.0;
            double matrix_elem_beta = 0.0;
# pragma omp parallel reduction(+:matrix_elem_alpha) reduction(+:matrix_elem_beta)
            for (size_t k = 0; k < num_basis; k++) {
              for (size_t l = 0; l < num_basis; l++) {
                ...
                if (quantum_part_a.num_parts == 1) {
                  matrix_elem_alpha += form_fock_elem(
                  ...
                  matrix_elem_beta += form_fock_elem(
                ...
                if (independent_converged &&
                  ...
                  matrix_elem_alpha += elem;
                  if (quantum_part_a.num_parts > 1 &&
                      quantum_part_a.restricted == false) {
                    matrix_elem_beta += elem;
                  }
                }
              }
            }
            this->F[quantum_part_a_idx][0](i, j) += matrix_elem_alpha;
            if (quantum_part_a.num_parts > 1 &&
                quantum_part_a.restricted == false) {
              this->F[quantum_part_a_idx][1](i, j) += matrix_elem_beta;
            }
          }
        }
        quantum_part_b_idx++;
