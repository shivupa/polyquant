...
#pragma omp parallel for schedule(runtime)
    for (auto quantum_part_a_idx = 0;
      ...
      for (size_t shell_i = 0; shell_i < num_shell_a; shell_i++) {
        for (size_t shell_j = 0; shell_j < num_shell_a; shell_j++) {
              ...
              for (size_t shell_l = 0; shell_l < num_shell_b; shell_l++) {
                ...
                if (this->Cauchy_Schwarz_screening) {
                  if (std::abs(Da_kl + Db_kl) *
                    ...
                    auto elements =
                        form_fock_helper(i, j, k, l, quantum_part_a_idx);
      ...
      void POLYQUANT_EPSCF::diag_fock() {
        ...
        for (auto const &[quantum_part_key, quantum_part] :
          ...
          Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> F_prime(
              num_basis, num_basis);
          ...
          if (this->incremental_fock) {
            if (this->iteration_rms_error[quantum_part_idx][0] <
                    this->incremental_fock_reset_threshold[quantum_part_idx]
                                                          [0] ||
                this->iteration_num -
              ...
              if (independent_converged_iteration_num < 0 &&
                ...
                buffer << "Starting incremental fock build for "
                       << quantum_part_key << " spin " << 0 << std::endl;
          ...
          F_prime = this->input_integral.orth_X * F_diis *
          ...
          if (quantum_part.num_parts > 1 && quantum_part.restricted == false) {
            ...
            Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>
                FD_commutator = this->F[quantum_part_idx][1] *
                                    this->D[quantum_part_idx][1] *
                                    this->input_integral.overlap -
                                this->input_integral.overlap *
                                    this->D[quantum_part_idx][1] *
                                    this->F[quantum_part_idx][1];
            ...
            if (this->incremental_fock) {
              if (this->iteration_rms_error[quantum_part_idx][1] <
                      this->incremental_fock_reset_threshold[quantum_part_idx]
                                                            [1] ||
                  this->iteration_num - this->incremental_fock_reset_iteration
                                            [quantum_part_idx][1] >
                      this->incremental_fock_reset_freq) {
                ...
                if (independent_converged_iteration_num < 0 &&
                  ...
                  std::stringstream buffer;
                  buffer << "Starting incremental fock build for "
                         << quantum_part_key << " spin " << 1 << std::endl;
                  Polyquant_cout(buffer.str());
                }
              }
            }
            if (this->diis_extrapolation) {
              this->diis[quantum_part_idx][1].extrapolate(F_diis,
                                                          FD_commutator);
            ...
            this->E_orbitals[quantum_part_idx][1] =
                eigensolver_beta.eigenvalues();
            C_prime = eigensolver_beta.eigenvectors();
            this->C[quantum_part_idx][1] =
                this->input_integral.orth_X * C_prime;
          }
          quantum_part_idx++;
        }
      }
...
#pragma omp parallel for
      ...
      void POLYQUANT_EPSCF::calculate_E_elec() {
        ...
        for (auto const &[quantum_part_key, quantum_part] :
          ...
          if (quantum_part.num_parts == 1) {
            ...
            this->E_particles[quantum_part_idx] =
                0.5 *
                (((this->D[quantum_part_idx][0] + this->D[quantum_part_idx][1])
                      .array() *
      ...
      void POLYQUANT_EPSCF::check_stop() {
        ...
        if (!this->independent_converged && this->converged && this->stop) {
          Polyquant_cout(
              "Independent densities converged. Turning on interactions.");
          this->converged = false;
          this->stop = false;
      ...
      void POLYQUANT_EPSCF::reset_diis() {
        if (this->diis_extrapolation) {
          ...
          libint2::DIIS<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>
              tempdiis(this->diis_start, this->diis_size, this->diis_damping, 1,
                       1, this->diis_mixing_fraction);
          auto quantum_part_idx = 0ul;
          for (auto const &[quantum_part_key, quantum_part] :
               this->input_molecule.quantum_particles) {
            if (quantum_part.num_parts == 1) {
        ...
        if (this->incremental_fock) {
          ...
          for (auto const &[quantum_part_key, quantum_part] :
            ...
            if (quantum_part.num_parts == 1) {
              ...
              this->incremental_fock_reset[quantum_part_idx][0] = true;
              this->incremental_fock_reset_threshold[quantum_part_idx].resize(
                  1);
              this->incremental_fock_reset_threshold[quantum_part_idx][0] =
                  this->incremental_fock_initial_onset_thresh;
              this->incremental_fock_reset_iteration[quantum_part_idx].resize(
                  1);
      ...
      void POLYQUANT_EPSCF::print_start_iterations() {
        ...
        buffer << "    diis_extrapolation" << this->diis_extrapolation
               << std::endl;
        ...
        buffer << "    Cauchy_Schwarz_screening"
               << this->Cauchy_Schwarz_screening << std::endl;
        buffer << "    Cauchy_Schwarz_threshold"
               << this->Cauchy_Schwarz_threshold << std::endl;
        Polyquant_cout(buffer.str());
        if (this->Cauchy_Schwarz_screening) {
...
#pragma omp parallel for
          ...
          if (quantum_part.num_parts > 1 && quantum_part.restricted == false) {
            Dataset = Super_Twist_group.get_dataset("eigenset_" +
                                                    std::to_string(idx + 1));
