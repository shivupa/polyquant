...
void POLYQUANT_INTEGRAL::construct_ericache(size_t size_in_gb) {
  ...
  polyquant_lfu_ericache<std::pair<std::vector<size_t>, std::vector<size_t>>, double, PairVectorHash<size_t>>
...
#pragma omp parallel for
    for (auto j = 0; j < mo_one_body_ints[quantum_part_idx].size(); j++) {
      ...
      mo_one_body_ints[quantum_part_idx][j] =
          ...
          (kinetic[quantum_part_idx] + (-quantum_part.charge * nuclear[quantum_part_idx])) *
...
double POLYQUANT_INTEGRAL::get2e_elem(const size_t& quantum_part_a_idx, const size_t& quantum_part_b_idx, const size_t& i, const size_t& j, const size_t& k, const size_t& l){
    if (quantum_part_a_idx > quantum_part_b_idx){
       if(i>j){
         std::vector<size_t> part_two = {quantum_part_a_idx, j, i};
       } else {
         std::vector<size_t> part_two = {quantum_part_a_idx, i, j};
       }
       if(k>l){
         std::vector<size_t> part_one = {quantum_part_b_idx, l, k};
       } else {
         std::vector<size_t> part_one = {quantum_part_b_idx, k, l};
       }
    } else {
       if(i>j){
         std::vector<size_t> part_one = {quantum_part_a_idx, j, i};
       } else {
       ...
       if(k>l){
         ...
         std::vector<size_t> part_two = {quantum_part_b_idx, k, l};
       }
    }

    std::pair<std::vector<size_t>, std::vector<size_t>> idx = std::make_pair(part_one, part_two);
    //finish by calculating entire shell if not present and storing entire shell..
...
//        val *= libint2::math::bc(angular_momentum_2[0], s_x_2);
    ...
    std::vector<size_t> part_one = {quantum_part_a_idx, i, j};
    std::vector<size_t> part_two = {quantum_part_b_idx, k, l};
    std::pair<std::vector<size_t>, std::vector<size_t>> idx = std::make_pair(part_a, part_b);
