...
extern "C" {
#endif
void _sphemultipole_L_S(const Libint_t* inteval) {

_libint2_static_api_bzero_short_(inteval->stack+0*1,1125)
const int contrdepth = inteval->contrdepth;
#ifdef __INTEL_COMPILER
#pragma ivdep
#endif
for(int c = 0; c<contrdepth; c++) {
_sphemultipole_L_S_prereq(inteval+c, inteval->stack);
}
...
inteval->targets[20] = &(inteval->stack[((hsi*45+900)*1+lsi)*1]);
inteval->targets[21] = &(inteval->stack[((hsi*45+945)*1+lsi)*1]);
inteval->targets[22] = &(inteval->stack[((hsi*45+990)*1+lsi)*1]);
inteval->targets[23] = &(inteval->stack[((hsi*45+1035)*1+lsi)*1]);
inteval->targets[24] = &(inteval->stack[((hsi*45+1080)*1+lsi)*1]);
/** Number of flops = 0 */
}
