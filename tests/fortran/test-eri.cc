...
extern "C" /* prevent C++ name mangling */
    ...
    compute_eri_f(int &contrdepth, int &deriv_order, int &am1, double *c1,
                  double *alpha1, double *A, int &am2, double *c2,
                  double *alpha2, double *B, int &am3, double *c3,
                  double *alpha3, double *C, int &am4, double *c4,
                  double *alpha4, double *D, double *F, Libint_t *erieval);

/// Independent C++ implementation of compute_eri, in order to check Fortran
/// result.
void compute_eri_c(int &contrdepth, int &deriv_order, int &am1, double *c1,
                   double *alpha1, double *A, int &am2, double *c2,
                   double *alpha2, double *B, int &am3, double *c3,
                   double *alpha3, double *C, int &am4, double *c4,
                   double *alpha4, double *D, double *F, Libint_t *erieval) {
      ...
      for (int p2 = 0; p2 < contrdepth; ++p2) {
        for (int p3 = 0; p3 < contrdepth; ++p3) {
          ...
          if (deriv_order > 0) {
            ...
            erieval[p0123].alpha1_rho_over_zeta2[0] =
                alpha1_ * gammapq / (gammap * gammap);
#endif
#if LIBINT2_DEFINED_alpha2_rho_over_zeta2
            erieval[p0123].alpha2_rho_over_zeta2[0] =
                alpha2_ * gammapq / (gammap * gammap);
#endif
#if LIBINT2_DEFINED_alpha3_rho_over_eta2
            erieval[p0123].alpha3_rho_over_eta2[0] =
                alpha3_ * gammapq / (gammaq * gammaq);
#endif
#if LIBINT2_DEFINED_alpha4_rho_over_eta2
            erieval[p0123].alpha4_rho_over_eta2[0] =
                alpha4_ * gammapq / (gammaq * gammaq);
#endif
#if LIBINT2_DEFINED_alpha1_over_zetapluseta
            erieval[p0123].alpha1_over_zetapluseta[0] =
                alpha1_ / (gammap + gammaq);
#endif
#if LIBINT2_DEFINED_alpha2_over_zetapluseta
            erieval[p0123].alpha2_over_zetapluseta[0] =
                alpha2_ / (gammap + gammaq);
#endif
#if LIBINT2_DEFINED_alpha3_over_zetapluseta
            erieval[p0123].alpha3_over_zetapluseta[0] =
                alpha3_ / (gammap + gammaq);
#endif
#if LIBINT2_DEFINED_alpha4_over_zetapluseta
            erieval[p0123].alpha4_over_zetapluseta[0] =
                alpha4_ / (gammap + gammaq);
#endif
          ...
          double pfac = 2 * pow(M_PI, 2.5) * K1 * K2 /
...
bool test_eri_c_f(int &contrdepth, int &am1, double *c1, double *alpha1,
                  double *A, int &am2, double *c2, double *alpha2, double *B,
                  int &am3, double *c3, double *alpha3, double *C, int &am4,
                  double *c4, double *alpha4, double *D, int &deriv_order,
  ...
  compute_eri_c(contrdepth, deriv_order, am1, c1, alpha1, A, am2, c2, alpha2, B,
                am3, c3, alpha3, C, am4, c4, alpha4, D, F, erieval_c);

  // Compute integrals using Fortran implementation
  compute_eri_f(contrdepth, deriv_order, am1, c1, alpha1, A, am2, c2, alpha2, B,
                am3, c3, alpha3, C, am4, c4, alpha4, D, F, erieval_f);
        ...
        for (int c = 0; c < n3; c++) {
          for (int d = 0; d < n4; d++, nel++) {
            ...
            if (rel_error > threshold) {
              std::cout << std::setprecision(17) << "Elem " << nel
                        << " di= " << deriv_order
                        << ", : C = " << *eri_shell_set_c
...
TEST_CASE("Fortran ERI", "[eri]") {
  ...
  unsigned int am[] = {(unsigned int)am1, (unsigned int)am2, (unsigned int)am3,
                       (unsigned int)am4};
