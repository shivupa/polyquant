...
TEST_SUITE("BASIS") {
  TEST_CASE("BASIS: Set up basis from library.") {
    ...
    CHECK(test_bas.num_basis[0] == 5);
    CHECK(test_bas.num_basis[1] == 2);
    CHECK(test_bas.basis[0][0].O[0] ==doctest::Approx(1.4304631499).epsilon(POLYQUANT_TEST_EPSILON_LOOSE));
    CHECK(test_bas.basis[1][0].O[0] ==doctest::Approx(1.4304631499).epsilon(POLYQUANT_TEST_EPSILON_LOOSE));
  }
