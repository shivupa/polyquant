...
TEST_CASE("MOLECULE: Construct Molecule.") {
  ...
  CHECK(test_mol.centers[0][0] == doctest::Approx(1.4304631499).epsilon(0.0001));
  CHECK(test_mol.charge == 0);
  CHECK(test_mol.multiplicity == 1);
  CHECK(test_mol.restricted == true);
  CHECK(test_mol.E_nuc == doctest::Approx(9.194863688325974).epsilon(0.0001));

  std::string xyz_ref = "3\nPOLYQUANT Dumped XYZ centers = all\nH    0.756969    0.000000    -0.585875\nH    -0.756969    0.000000    -0.585875\nO    0.000000    0.000000    0.000000";
...
TEST_CASE("MOLECULE: Construct Molecule with no keywords"){
  POLYQUANT_INPUT test_inp(
      "../../tests/unit_tests/data/h2o_sto3glibrary/h2o_nokeyword.json");
...
TEST_CASE("MOLECULE: Construct Molecule with quantum nuclei specificed by center list.") {
  POLYQUANT_INPUT test_inp(
      "../../tests/unit_tests/data/h2o_sto3g_quantumH/h2o.json");
  POLYQUANT_MOLECULE test_mol(test_inp);
  CHECK(test_mol.centers[1][0] == doctest::Approx(1.4304631499).epsilon(0.0001));
  CHECK(test_mol.charge == 0);
  CHECK(test_mol.multiplicity == 1);
  CHECK(test_mol.restricted == false);
}
TEST_CASE("MOLECULE: Construct Molecule with quantum nuclei specificed by center label.") {
  POLYQUANT_INPUT test_inp(
      "../../tests/unit_tests/data/h2o_sto3g_quantumH/h2o_alternate.json");
  POLYQUANT_MOLECULE test_mol(test_inp);
  CHECK(test_mol.centers[1][0] == doctest::Approx(1.4304631499).epsilon(0.0001));
  CHECK(test_mol.charge == 0);
  CHECK(test_mol.multiplicity == 1);
...
TEST_CASE("MOLECULE: Construct Molecule with positron"){
  POLYQUANT_INPUT test_inp(
      "../../tests/unit_tests/data/li-_custombasis_wpos/Li_wpos.json");
  POLYQUANT_MOLECULE test_mol(test_inp);
