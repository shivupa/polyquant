...
TEST_CASE("CI: same part ham diag ") {
  ...
  CHECK(diag_ham_elem == 
        doctest::Approx(-84.1577927627923).epsilon(POLYQUANT_TEST_EPSILON_LOOSE));
}
TEST_CASE("CI: same part ham single ") {
  ...
  CHECK(off_diag_ham_single_elem == 
        doctest::Approx(3.042582036923841e-08).epsilon(POLYQUANT_TEST_EPSILON_LOOSE));
...
TEST_CASE("CI: same part ham double ") {
  ...
  CHECK(off_diag_ham_double_elem == 
        doctest::Approx(0.010855436090541142).epsilon(POLYQUANT_TEST_EPSILON_LOOSE));
